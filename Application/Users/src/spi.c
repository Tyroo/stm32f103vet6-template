...
// 使能SPI1和GPIOA中断
	...
	SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;													// 设置SPI时钟在低电平时为空闲状态
...
bool SPI1_Send(u8* SendData) {
	...
	FlagStatus SendFlag = SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE);	// SPI发送缓冲区空标志
	
	// 等待发送缓冲区清空
	while(SendFlag == RESET);
...
void SPI1_IRQHandler() {
	...
	// SPI接收缓冲区非空标志
	FlagStatus ReadFlag = SPI_I2S_GetITStatus(SPI1, SPI_I2S_IT_TXE);
